...
int main(int argc, char *argv[]) {
  ...
  cv::namedWindow("Video", cv::WINDOW_AUTOSIZE | cv::WINDOW_GUI_NORMAL);
  static int image_count = 0;
  static int image_res_count = 0;
  TimeDuration time_duration;
  Timeout timeout(std::chrono::seconds(36000));
    ...
    auto func = [&] {
      ...
      if (image_res != nullptr) {
        ...
        KAYLORDUT_LOG_INFO(
            ...
            duration.count());
	int win_width = image_res->cols;
	int win_height = image_res->rows;
